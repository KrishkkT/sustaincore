...
`;document.addEventListener("DOMContentLoaded",()=>{const r=document.getElementById("calc-netmass"),o=document.getElementById("calc-see"),s=document.getElementById("calc-benchmark"),t=document.getElementById("calc-cost"),e=document.querySelector(".input-group select"),n=document.getElementById("out-netmass"),d=document.getElementById("out-emissions"),a=document.getElementById("out-phaseout"),i=document.getElementById("out-cost"),l={"Iron & Steel (72, 73)":1.6,"Aluminium (76)":2.1,"Cement (2523)":.8,"Fertilizers (3102, 3105)":1.9,"Hydrogen (2804 10 00)":10.4},m=.975,g={EUR:1,USD:1.09,INR:90};let h="EUR";const x=c=>(h==="EUR"?"€":h==="USD"?"$":"₹")+Math.round(c).toLocaleString(),v=()=>{const c=parseFloat(r.value)||0,u=parseFloat(o.value)||0,p=parseFloat(s.value)||0,k=parseFloat(t.value)||0,f=c*u,y=p*c*m,E=Math.max(0,f-y)*k;n.innerText=c.toLocaleString()+" t",d.innerText=Math.round(f).toLocaleString()+" tCO₂",a.innerText=(m*100).toFixed(2)+"%";const S=E*g[h];i.innerText=x(S)};[r,o,s,t].forEach(c=>{c&&c.addEventListener("input",v)}),e&&e.addEventListener("change",c=>{const u=l[c.target.value];u&&(s.value=u,v())});const b=document.querySelector(".calc-results");if(b){const c=document.createElement("div");c.className="currency-toggle flex gap-2 mt-4",c.innerHTML=`
        ...
        `,b.appendChild(c),c.querySelectorAll("button").forEach(u=>{u.addEventListener("click",()=>{c.querySelectorAll("button").forEach(p=>p.classList.remove("active","bg-brand-blue","text-white")),u.classList.add("active","bg-brand-blue","text-white"),h=u.getAttribute("data-cur"),v()})}),c.querySelector('[data-cur="EUR"]').classList.add("bg-brand-blue","text-white")}v()});function B(){const r=new Date,o=r.getMonth(),s=r.getFullYear();let t,e=s;o<3?t=3:o<6?t=6:o<9?t=9:(t=0,e=s+1);const d=[{m:0,d:31},{m:3,d:30},{m:6,d:31},{m:9,d:31}].find(a=>a.m===t);return new Date(e,d.m,d.d,23,59,59)}function w(){const s=B()-new Date;if(s<=0)return;const t=Math.floor(s/(1e3*60*60*24)),e=Math.floor(s%(1e3*60*60*24)/(1e3*60*60)),n=Math.floor(s%(1e3*60*60)/(1e3*60)),d=Math.floor(s%(1e3*60)/1e3);document.getElementById("days").textContent=t.toString().padStart(2,"0"),document.getElementById("hours").textContent=e.toString().padStart(2,"0"),document.getElementById("minutes").textContent=n.toString().padStart(2,"0"),document.getElementById("seconds").textContent=d.toString().padStart(2,"0")}document.addEventListener("DOMContentLoaded",()=>{document.getElementById("cbam-timer")&&(w(),setInterval(w,1e3))});document.addEventListener("DOMContentLoaded",()=>{const r=document.getElementById("header-root"),o=document.getElementById("footer-root");r&&(r.innerHTML=L),o&&(o.innerHTML=C);const s=document.getElementById("theme-toggle"),t=document.documentElement;(localStorage.getItem("sustain-theme")||"light")==="dark"?(t.classList.add("dark"),s&&s.setAttribute("aria-checked","true")):(t.classList.remove("dark"),s&&s.setAttribute("aria-checked","false")),s&&s.addEventListener("click",function(){t.classList.contains("dark")?(t.classList.remove("dark"),s.setAttribute("aria-checked","false"),localStorage.setItem("sustain-theme","light")):(t.classList.add("dark"),s.setAttribute("aria-checked","true"),localStorage.setItem("sustain-theme","dark"))}),(()=>{const a=document.getElementById("mobile-menu-btn"),i=document.getElementById("close-menu"),l=document.getElementById("mobile-menu");a&&l&&a.addEventListener("click",()=>{l.classList.remove("translate-x-full")}),i&&l&&i.addEventListener("click",()=>{l.classList.add("translate-x-full")}),document.querySelectorAll("#mobile-menu a").forEach(x=>{x.addEventListener("click",()=>{l&&l.classList.add("translate-x-full")})});const g=document.getElementById("mobile-services-toggle"),h=document.getElementById("mobile-services-list");g&&h&&g.addEventListener("click",()=>{h.classList.toggle("hidden"),g.querySelector("svg").classList.toggle("rotate-180")})})(),document.querySelectorAll(".flip-card").forEach(a=>{a.addEventListener("click",i=>{window.matchMedia("(max-width: 1023px)").matches&&a.classList.toggle("is-flipped")})})});document.addEventListener("DOMContentLoaded",()=>{const r=document.querySelectorAll(".tab-btn"),o=document.querySelectorAll(".panel"),s=document.querySelectorAll(".mega-item"),t=()=>{const a=document.getElementById("services-scroll-target")||document.getElementById("services-nav-anchor");a&&a.scrollIntoView({behavior:"smooth",block:"start"})},e=a=>{r.forEach(i=>{i.getAttribute("data-tab")===a?i.classList.add("active"):i.classList.remove("active")}),o.forEach(i=>{i.id===a?i.classList.add("active"):i.classList.remove("active")})};r.forEach(a=>{a.addEventListener("click",i=>{const l=a.getAttribute("data-tab");e(l),t(),history.pushState(null,null,`#${l}`)})}),s.forEach(a=>{a.addEventListener("click",i=>{const l=a.getAttribute("href");if(l&&l.includes("#")){const m=l.split("#")[1];window.location.pathname.includes("services.html")&&(i.preventDefault(),e(m),t(),history.pushState(null,null,`#${m}`))}})});const n=()=>{const a=window.location.hash.substring(1);if(a)document.querySelector(`.tab-btn[data-tab="${a}"]`)&&(e(a),setTimeout(t,100));else if(r.length>0){const i=document.querySelector(".tab-btn.active");if(i){const l=i.getAttribute("data-tab");e(l)}}};window.addEventListener("load",n),window.addEventListener("hashchange",n),document.querySelectorAll(".faq-btn, .sp-faq-btn").forEach(a=>{a.addEventListener("click",()=>{const i=a.parentElement;document.querySelectorAll(".faq-item.active, .sp-faq-item.active").forEach(l=>{l!==i&&l.classList.remove("active")}),i.classList.toggle("active")})})});const M=()=>{let r=document.querySelector(".toast-container");return r||(r=document.createElement("div"),r.className="toast-container",document.body.appendChild(r)),r};window.showToast=(r,o,s="success")=>{const t=M(),e=document.createElement("div");e.className=`toast toast-${s}`;const n=s==="success"?'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>':'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>';e.innerHTML=`
    ...
    `,t.appendChild(e),setTimeout(()=>e.classList.add("show"),100);const d=()=>{e.classList.remove("show"),setTimeout(()=>e.remove(),600)},a=setTimeout(d,5e3);e.querySelector(".toast-close").addEventListener("click",()=>{clearTimeout(a),d()})};window.handleSubmit=async function(r){r.preventDefault();const o=r.target,s=new FormData(o),t=o.querySelector('button[type="submit"]'),e=t.innerHTML;t.disabled=!0,t.innerHTML="Sending...";try{const n=await fetch(o.action,{method:o.method,body:s,headers:{Accept:"application/json"}});if(n.ok)window.showToast("Message Sent","Thank you! Our technical team will reach out shortly.","success"),o.reset();else{const d=await n.json();window.showToast("Submission Error",d.errors?d.errors[0].message:"Unable to send message. Please try again.","error")}}catch{window.showToast("System Offline","Could not connect to the server. Please check your internet.","error")}finally{t.disabled=!1,t.innerHTML=e}};
